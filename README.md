...
不想装扩展？直接用浏览器打开 [`demo/index.html`](demo/index.html) 即可模拟**网格 / 时间线 / 时间筛选**效果（无需登录、无需安装）。

> 如果你在 GitHub 上开启了 **Pages**（Settings → Pages → 选择 `main` 分支根目录），这个 `demo/` 会自动变成公开的在线演示页：`https://<你的用户名>.github.io/xiaohongshu-timeline/demo/`。
...
├── demo/
│   └── index.html         # 免安装模拟（网格 / 时间线 / 时间筛选）
├── tools/
│   └── diagnose.js        # 数据诊断脚本（排查抓取 / 时间解析问题）
├── assets/
│   ├── logo.svg
│   └── preview.svg
├── LICENSE
...
`tools/diagnose.js` 是一个**独立诊断脚本**，当你怀疑「抓取不全」或「时间解析错了」时，把它粘进个人主页的 Console 回车，刷新并滚动页面，控制台会打印**按月时间分布**、`time` 原始值抽样、以及话题（tag_list）统计，方便定位问题。它不依赖扩展本身。

## 🚧 已知限制

- 小红书前端改版频繁，若接口结构或 DOM 类名大幅变化，扩展可能暂时失效，需跟进适配。
- 筛选范围受「已加载笔记」约束（见上文）。
- 演示页 `demo/index.html` 额外展示了「话题」分组视图——这是**规划中的功能**，当前扩展版本（v0.2.6）仅实装网格 + 时间线 + 时间筛选。

## 🗺️ 路线图

- [ ] 关键词 / 语义聚类（本地 Embedding 或 LLM API）
- [ ] 筛选器增强：只看视频 / 图文、按点赞区间过滤
- [ ] 导出 Markdown / CSV 笔记列表
- [ ] 自适应暗黑模式
- [ ] 自动滚动加载更多（无需手动滚动）
